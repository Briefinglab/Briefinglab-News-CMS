...
class Bl_News_Cms_Manager {
    ...
    public function __construct() {
        ...
        $this->plugin_slug = 'bl-news-cms';
        $this->version = '1.0.0';
        $this->options = array(
            'bl-news-custom-template-folder' => 'bl-custom-templates'
        ); // now fixed
    ...
    private function load_dependencies() {
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-bl-news-cms-model.php';
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-bl-news-cms-manager-admin.php';
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-bl-news-cms-manager-options.php';
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-bl-news-cms-manager-public.php';
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-bl-cache-html-manager.php';

        require_once plugin_dir_path( __FILE__ ) . 'class-bl-news-cms-loader.php';
        $this->loader = new Bl_News_Cms_Loader();
    ...
    private function define_admin_hooks() {
        ...
        $data_model = Bl_News_Cms_Model::getInstance();

        $upload_dir = wp_upload_dir();

        $upload_dir = $upload_dir['basedir'] . "/bl-news/";

        $cache_manager = new Bl_Cache_Html_Manager( $upload_dir );

        $admin = new Bl_News_Cms_Manager_Admin( $this->get_version(), $this->options, $data_model, $cache_manager );

        $this->loader->add_action( 'init', $admin, 'load_textdomain' );
        $this->loader->add_action( 'init', $admin, 'register_bl_news_post_type' );
...
/**
    ...
    private function define_public_hooks() {
        ...
        $public = new Bl_News_Cms_Manager_Public( $this->get_version(), $this->options, $data_model, $cache_manager);

        $this->loader->add_action( 'init', $public, 'create_shortcode_bl_news' );
